...
let main = () => {
  Browser.window->P5.setSetup(() => {
    ...
    P5.text("Uniform Distribution (ReScript + p5.js)", 2.0 * scaleFactor, 6.0 * scaleFactor)
  })
  ...
  Browser.window->P5.setDraw(() => {
    ...
    let _ = randomCounts->Array.mapWithIndex((count, index) => {
      P5.rect(Int.toFloat(index) * w, 
      screenHeight - Int.toFloat(count), w - 1.0, Int.toFloat(count))
    })
  })
